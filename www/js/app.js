var app = new Vue({
    ...
    methods: {
        ...
        modal: function (act, type, val1, val2) {
            ...
            switch (act) {
                ...
                case 'open':
                    ...
                    switch (type) {
                        case 'add':
                            ...
                            switch (t.temp.mode) {
                                ...
                                case 'carry':
                                    t.temp.modal.sequence = t.get('sequence');
                                    t.temp.modal.carry.value = '';
                                    inputType = 'number';
                                    break;
                            }

                            t.temp.modal.el.on('shown.bs.modal', function () {
                        ...
                        case 'edit':
                            ...
                            switch (t.temp.mode) {
                                case 'record':
                                    ...
                                    t.temp.modal.el.off('shown.bs.modal');
                                    break;
                                ...
                                case 'carry':
                                    ...
                                    t.temp.modal.carry['u'] = carry.u;
                                    t.temp.modal.carry['year'] = carry.year;
                                    t.temp.modal.carry['value'] = carry.value;
                    ...
                    t.temp.modal.el.modal('show');
                    break;

                case 'close':
                    t.temp.modal.el.modal('hide');
                    break;
                    ...
                    switch (t.temp.mode) {
                        ...
                        case 'kind':
                            if (!t.temp.modal.kind.value) {
                                ...
                                appLib.bandMessage('warning', '입력하신 D-Day 값이 올바르지 않습니다.', 3000);
                                return;
                            }
                        ...
                        case 'carry':
                            var carries = t.protected[t.private.idx].carries;

                            if (!t.temp.modal.carry.year) {
                                appLib.bandMessage('warning', '연도를 선택해주세요.', 3000);
                                return;
                            }
                            ...
                            switch (t.temp.modal.type) {
                                case 'add':
                                    for (var i in carries) {
                                        if (carries[i].year === t.temp.modal.carry.year) {
                                            appLib.bandMessage('warning', '선택하신 연도의 데이터가 이미 있습니다.', 3000);
                                            return;
                                        }
                                    }

                                    t.protected[t.private.idx].carries.push({
                                        u: t.temp.modal.sequence,
                                        year: t.temp.modal.carry.year,
                                        value: Number(t.temp.modal.carry.value)
                                    });
                                    break;

                                case 'edit':
                                    var carry = t.carry('get', t.temp.modal.carry.u);
                                    ...
                                    for (var i in carries) {
                                        if (carries[i].u !== carry.u && carries[i].year === t.temp.modal.carry.year) {
                                            appLib.bandMessage('warning', '선택하신 연도의 데이터가 이미 있습니다.', 3000);
                                            return;
                                        }
                                    }

                                    carry.year = t.temp.modal.carry.year;
                                    carry.value = Number(t.temp.modal.carry.value);
                                    break;
                    ...
                    switch (type) {
                        case 'load':
                            if (t.temp.modal.histories.length) {
                                ...
                                if (!t.temp.modal.histories.length)
                                    appLib.bandMessage('warning', '데이터가 없습니다.', 3000);
                            }
                            break;
                    }
                    break;
                ...
                case 'delete':
                    if (confirm('삭제하시겠습니까?')) {
                        switch (t.temp.mode) {
                            ...
                            case 'carry':
                                for (var i in t.protected[t.private.idx].carries) {
                                    if (t.protected[t.private.idx].carries[i].u === t.temp.modal.carry.u) {
                                        t.protected[t.private.idx].carries.splice(i, 1)
                                        break;
                                    }
                                }
        ...
        record: function (act, val1, val2) {
            ...
            switch (act) {
                case 'calc':
                    ...
                    var thisYear = Number(thisDate.toString().substring(0, 4));
                    ...
                    for (var i in carries) {
                        if (carries[i].year && carries[i].year === thisYear) {
                            carried += carries[i].value;
                            break;
                        }
                    }
                                ...
                                t.temp.modal.box = bootbox.prompt({
                                    ...
                                    callback: function (newVal) {
                                        if (!newVal) {
                                            ...
                                            appLib.bandMessage('success', '추가하였습니다.', 3000);
                                        }
                                    }
                                });
                            }
        ...
        carry: function (act, val1, val2) {
            switch (act) {
                case 'get':
                    if (this.protected[this.private.idx].carries) {
                        for (var i in this.protected[this.private.idx].carries) {
                            if (this.protected[this.private.idx].carries[i].u === val1)
                                return this.protected[this.private.idx].carries[i];
                        }
                    }
                    break;
                ...
                default:
                    ...
                    this.temp.modal.carry.year = year;
                    this.temp.modal.carry.years = [];
                    this.temp.mode = 'carry';
                    this.nav('close');
                    this.modal('close');

                    for (var i = 5; i > 0; i -= 1)
                        this.temp.modal.carry.years.push(year - i);
    ...
    computed: {
        ...
        carries: function () {
            ...
            return this.protected[this.private.idx].carries.sort(compare);
        },
        now: function () {
            return appLib.now();
        }
    ...
    created: function () {
        ...
        if (private)
            t.private = JSON.parse(private);
        else
            t.private = defaultData.private
        ...
        if (protected)
            ...
            t.protected = defaultData.protected;
        ...
        // 데이터 추가. 2019-02-03
        for (var i in t.protected) {
            if (!t.protected[i]['carries'])
                t.protected[i]['carries'] = [];
        }
        ...
        document.addEventListener('deviceready', function () {
            ...
            document.addEventListener('backbutton', function () {
                ...
                if (t.temp.nav.visible) {
                    ...
                    if (!t.temp.exitCheck) {
                        appLib.bandMessage('warning', '한번 더 누르시면 종료합니다.', 3000);
    ...
    mounted: function () {
        this.temp.modal.el = $(this.$el).find('#appModal');
    }
});